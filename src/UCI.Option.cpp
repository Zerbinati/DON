...
namespace UCI {
    ...
    Option& CheckOption::operator= (string &value)
    {
        ...
        bool val = (value == "true" || value == "True");
        if (_value != val)
        {
            _value = val;
            if (_on_change != nullptr) _on_change (*this);
    ...
    SpinOption::SpinOption (const i32 val, i32 minimum, i32 maximum, const OnChange on_change)
        : Option (on_change)
        , _default (val)
        , _value (val)
        , _minimum (minimum)
...
#   endif
    ...
    void   initialize ()
    {
        ...
        Options["Never Clear Hash"]             = OptionPtr (new CheckOption (false, on_never_clear_hash));
        ...
        Options["Auto Save Hash (min)"]         = OptionPtr (new SpinOption ( 0, 0, 60));
        ...
        Options["Opening Book"]                 = OptionPtr (new StringOption ("", on_change_book));
        ...
        Options["Threads"]                      = OptionPtr (new SpinOption ( 1, 1, MaxThreads, on_config_threadpool));
        ...
        Options["Split Depth"]                  = OptionPtr (new SpinOption ( 0, 0, MaxSplitDepth, on_config_threadpool));
        ...
        Options["Skill Level"]                  = OptionPtr (new SpinOption (MaxSkillLevel,  0, MaxSkillLevel));
        ...
        //Options["MultiPV_cp"]                   = OptionPtr (new SpinOption (0, 0, 1000));
        ...
        Options["Fixed Contempt"]               = OptionPtr (new SpinOption (0,-100,+100));
        // Time (sec) for Timed Contempt
        // Default +6, Min 0, Max +900.
        Options["Timed Contempt (sec)"]         = OptionPtr (new SpinOption (+ 20,   0,+900));
        // Centipawn (cp) for Valued Contempt
        // Default +50, Min 0, Max +1000.
        Options["Valued Contempt (cp)"]         = OptionPtr (new SpinOption (+ 50,   0,+1000));
        ...
        Options["Space"]                        = OptionPtr (new SpinOption ( 0,-1000,+1000, on_change_evaluation));
        Options["King Safety"]                  = OptionPtr (new SpinOption ( 0,-1000,+1000, on_change_evaluation));
        ...
        Options["Slowness"]                     = OptionPtr (new SpinOption (+ 85,+ 10,+ 1000));

        // Debug Options
        // -------------
        // The filename of the debug log.
        Options["Debug Log"]                    = OptionPtr (new StringOption ("", on_debug_log));
        // The filename of the search log.
        Options["Search Log"]                   = OptionPtr (new StringOption (""));
        ...
        //Options["UCI_ELO"]                      = OptionPtr (new SpinOption ( 0, 0, 3000));

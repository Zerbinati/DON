...
void PGN::close () { if (is_open ()) { fstream::close (); reset (); } }
...
void PGN::build_indexes ()
{
    if (is_open () && (_mode & ios_base::in) && good ())
    {
        if (0 < game_count ()) reset ();
...
void PGN::scan_index (const char *buf, u64 &pos, PGN_State &pgn_state)
{
    ASSERT (buf);

        ...
        private:
            ...
            void initialize (const Position &pos)
            {
                ...
                if (pinned_pawns != 0)
                {
                    ...
                    Bitboard pawn_attacks = shift_bb<LCap> (loosed_pawns)
        ...
        Score evaluate_pieces (const Position &pos, EvalInfo &ei, const Bitboard mobility_area, Score &mobility)
        {
            ...
            for (Square s : pos.squares<PT> (Own))
            {
                ...
                if (PT == QUEN)
                {
                    ...
                    Bitboard pinners;
                    b = pos.slider_blockers (s, pos.pieces (Own), pos.pieces (Opp, BSHP, ROOK), pinners);
                    if (b != 0)
                    {
                        score -= QueenWeaken;
                    }
        ...
        Score evaluate_passed_pawns (const Position &pos, const EvalInfo &ei)
        {
            ...
            while (passers != 0)
            {
                ...
                if (rr != 0)
                {
                    ...
                    eg_value +=
                          5*rr*dist (pos.square<KING> (Opp), push_sq)
                        - 2*rr*dist (pos.square<KING> (Own), push_sq);
                    // If block square is not the queening square then consider also a second push.
                    if (rel_rank (Own, push_sq) != R_8)
                    {
                        eg_value +=
                            - 1*rr*dist (pos.square<KING> (Own), push_sq+Push);
                    }

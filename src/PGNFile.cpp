...
PGNFile::PGNFile (const string &pgn_fn, ios_base::openmode mode)
...
bool PGNFile::open (const string &pgn_fn, ios_base::openmode mode)
{
    close ();
...
void PGNFile::close () { if (is_open ()) { fstream::close (); _reset (); } }
...
void PGNFile::_scan_index (const string &str, u64 &pos, State &state)
{
    size_t offset = 0;
...
void PGNFile::_add_index (u64 pos)
{
...
string PGNFile::read_text (u64 index)
{
...
string PGNFile::read_text (u64 beg_index, u64 end_index)
{
...
u64    PGNFile::write_text (const string &text)
{
    if (is_open () && good ())
    {
        *this << text;
...
Game   PGNFile::read_game (u64 index)
{
    return Game (read_text (index));
}
// Write the game and return index of the game
u64    PGNFile::write_game (const Game &game)
{
    // TODO::
    string pgn = game.pgn ();
    *this << pgn;

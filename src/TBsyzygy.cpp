...
#           endif
        ...
        struct TBTable
        {
            typedef typename std::conditional<WDL == Type, WDLScore, i32>::type Ret;
            ...
            explicit TBTable (std::string const&);
        ...
        template<>
        TBTable<WDL>::TBTable (std::string const &code)
            : TBTable ()
        {
            StateInfo si;
            Position pos;
        ...
        {
        private:
            static i32 const Size = 1 << 12; // 4K table, indexed by key's 12 lsb
